...
your_account_label (new QLabel ("Your Qlc account:")),
...
void rai_qt::self_pane::refresh_balance ()
{
	...
	if (!error)
	{
		...
		for (auto & info : infos)
		{
			...
			{
				final_text += " Pending: " + wallet.format_balance (balance.second, info.token_type);
			}
			final_text += "\n";
		}
	}
...
void rai_qt::accounts::refresh_wallet_balance ()
{
	...
	std::map<rai::block_hash, std::pair<rai::uint128_t, rai::uint128_t>> balances;
		...
		if (!error && !infos.empty ())
		{
			for (auto & info : infos)
			{
				...
				if (balances.find (token_type) != balances.end ())
				{
					...
					balances[token_type].second += pending;
				}
				else
				{
					balances[token_type] = std::make_pair (balance, pending);
				}
			}
		}
	}
	...
	{
		final_text = QString ("Total(%1): [Balance] -> %2").arg (QString (rai::get_sc_info_name (b.first).c_str ()), QString (wallet.format_balance (b.second.first, b.first).c_str ()));
		auto p (b.second.second);
		if (!p.is_zero ())
		{
			final_text += QString ("; [Pending] -> %1").arg (QString (wallet.format_balance (p, b.first).c_str ()));
		}
	}
	wallet_balance_label->setText (final_text);
	...
	for (auto i (wallet.wallet_m->store.begin (transaction)), j (wallet.wallet_m->store.end ()); i != j; ++i)
	{
		...
		for (auto entry = rai::map_sc_info.begin (); entry != rai::map_sc_info.end (); ++entry)
		{
			...
			if (display)
			{
				...
				std::string balance = wallet.format_balance (balance_amount, entry->first);
				...
				auto token (new QStandardItem (QString ("%1[%2]").arg (QString (rai::get_sc_info_name (info.token_type).c_str ()), QString (info.token_type.to_string ().c_str ()))));
...
void rai_qt::history::refresh ()
{
	...
	for (auto & info : infos)
	{
		...
		for (auto i (0), n (tx_count->value ()); i < n && !hash.is_zero (); ++i)
		{
			...
			auto balanceItem = new QStandardItem (QString (wallet.format_balance (visitor.amount, info.token_type).c_str ()));
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			std::vector<rai::account_info> infos;
			rai::transaction transaction (this->wallet.node.store.environment, nullptr, false);
			this->wallet.node.store.accounts_get (transaction, account, infos);
			std::string final_text ("");
			for (const auto & info : infos)
			{
				...
				if (!balance.second.is_zero ())
				{
					final_text += "\nPending: " + wallet.format_balance (balance.second, info.token_type);
				}
			}
...
abi_window (new QWidget),
abi_layout (new QHBoxLayout),
abi_path_label (new QLabel ("Smart Contract abi file path: ")),
abi_path_line (new QLineEdit),
abi_choose_file (new QPushButton ("Choose")),
...
wallet (wallet_a)
{
	...
	abi_path_line->setReadOnly (true);
	abi_layout->addWidget (abi_path_line, 9);
	abi_layout->addWidget (abi_choose_file, 1);
	abi_layout->setContentsMargins (0, 0, 0, 0);
	abi_window->setLayout (abi_layout);
	//layout->addWidget (abi_path_line);
	layout->addWidget (abi_window);
	...
	QObject::connect (abi_choose_file, &QPushButton::released, [this]() {
		QString filename = QFileDialog::getOpenFileName (
		this->window, "Open ABI file",
		QDir::currentPath (), "All files (*.*) ;; ABI file (*.abi)");

		if (!filename.isNull ())
		{
			this->abi_path_line->setText (filename);
	...
	QObject::connect (publish, &QPushButton::released, [this]() {
		...
		if (!sc_account.decode_account (sc_account_text_narrow))
		{
			...
			if (!sc_owner_account.decode_account (sc_owner_account_text_narrow))
			{
				...
				try
				{
					stream.open (abi_file_text_narrow.c_str (), std::ios::in);
				}
				...
				if (!stream.fail ())
				{
					...
					rai::transaction transaction (this->wallet.wallet_m->node.store.environment, nullptr, true);
					rai::raw_key key;
					if (!wallet.wallet_m->store.fetch (transaction, sc_owner_account, key))
					{
						...
						assert (!error);*/
						auto work (this->wallet.wallet_m->node.work_generate_blocking (sc_owner_account));
						rai::smart_contract_block sc_block (sc_account, sc_owner_account, abi, key, sc_owner_account, work);
						...
						attempt->push_smart_contract (lock);*/
					}
				}
				else
				{
...
smart_contract_button (new QPushButton ("Publish Smart Contract"))
{
	...
	for (auto it = rai::map_sc_info.begin (); it != rai::map_sc_info.end (); ++it)
	{
		...
		it->first.encode_hex (token_hash);
		send_token_type->addItem (QString::fromStdString (name), QString::fromStdString (token_hash));
	};
		...
		if (auto this_l = this_w.lock ())
		{
			...
			if (!amount.decode_dec (this_l->send_count->text ().toStdString ()))
			{
				...
				if (actual / this_l->rendering_ratio == amount.number ())
				{
					...
					auto index (this_l->send_token_type->currentIndex ());
					auto token_type_text (this_l->send_token_type->itemData (index).toString ());
					std::string token_type_text_narrow (token_type_text.toLocal8Bit ());
					token_type.decode_hex (token_type_text_narrow);
					qDebug () << "Token: " << token_type_text_narrow.c_str ();
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, account_a]() {
				if (auto this_l = this_w.lock ())
				{
					...
					rai::account_info info;
					auto error (this_l->wallet_m->node.store.account_get (transaction, account_a, info));
					if (!error)
					{
						if (info.account == this_l->account)
						{
...
std::string rai_qt::wallet::format_balance (rai::uint128_t const & balance, rai::block_hash const & token_type) const
{
	auto sc_info (rai::get_sc_info (token_type));
	if (!sc_info.empty ())
	{
		std::vector<std::string> v{ std::begin (sc_info), std::end (sc_info) };
		rai::uint128_t rendering_ratio (v[rai::smart_contract_field::scale]);
...
wallet (wallet_a)
{
	...
	ledger_model->setHorizontalHeaderItem (3, new QStandardItem ("Block"));
...
void rai_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.latest_begin (transaction)), j (wallet.node.ledger.store.latest_end ()); i != j; ++i)
	{
		...
		items.push_back (new QStandardItem (QString (info.account.to_account ().c_str ())));
		items.push_back (new QStandardItem (QString (rai::get_sc_info_name (info.token_type).c_str ())));
		std::string balance;
