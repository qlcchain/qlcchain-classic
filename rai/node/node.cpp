...
receive_minimum (rai::qlc_ratio),
online_weight_minimum (60000 * rai::Gqlc_ratio),
...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::qlc_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
...
rai::process_return rai::block_processor::process_receive_one (MDB_txn * transaction_a, std::shared_ptr<rai::block> block_a)
{
	...
	switch (result.code)
	{
		...
		case rai::process_result::sc_account_mismatch:
		{
			if (node.config.logging.ledger_logging ())
			{
				BOOST_LOG (node.log) << boost::str (boost::format ("Smart contract account mismatch for: %1%") % hash.to_string ());
			}
			break;
		}
		case rai::process_result::abi_already_exist:
		{
			if (node.config.logging.ledger_logging ())
			{
				BOOST_LOG (node.log) << boost::str (boost::format ("Smart contract abi already exist for: %1%") % hash.to_string ());
			}
			break;
		}
		case rai::process_result::abi_mismatch:
		{
			if (node.config.logging.ledger_logging ())
			{
				BOOST_LOG (node.log) << boost::str (boost::format ("Smart contract abi mismatch for: %1%") % hash.to_string ());
			}
			break;
		}
		...
		default:
		{
			if (node.config.logging.ledger_logging ())
			{
				BOOST_LOG (node.log) << boost::str (boost::format ("Unknow process result of %1%") % block_a->hash ().to_string ());
			}
			break;
		}
	}
...
stats (config.stat_config)
{
	...
	{
		unsigned char rai_bootstrap_weights[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd5, 0xba, 0x6c, 0x7b, 0xb3, 0xf4, 0xf6, 0x54, 0x5e, 0x08, 0xb0, 0x3d, 0x6d, 0xa1, 0x25, 0x88, 0x40, 0xe0, 0x39, 0x50, 0x80, 0x37, 0x8a, 0x89, 0x06, 0x01, 0x99, 0x1a, 0x2a, 0x9e, 0x31, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd5, 0x29, 0xae, 0x9e, 0x86, 0x00, 0x00, 0x00 };
		...
		if (!rai::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mqlc_ratio, 0, true) << " QLC";
...
{
	assert (balance_a >= amount_a * rai::Gqlc_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= rai::Gqlc_ratio;
		auto balance_scaled ((balance_l / rai::Mqlc_ratio).convert_to<double> ());
...
void rai::online_reps::vote (std::shared_ptr<rai::vote> const & vote_a)
{
	...
	if (rep_it == reps.get<1> ().end ())
	{
		...
		{
			// overflow
			online_stake_total = rai::genesis_amount;
		}
		reps.insert (info);
	}
	else
